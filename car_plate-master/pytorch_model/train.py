...
class CarPlateLoader(Dataset):
    ...
    def __getitem__(self, item):
        ...
        temp = np.fromfile(PICS_PATH + "/" + self.pics[item], dtype=np.uint8)
        img = cv.imdecode(temp, -1)
        # print(PICS_PATH + "/" + self.pics[item])
        r, g, b = cv.split(img)
...
def main(args):
    ...
    data_loader = DataLoader(data_set, batch_size=50, shuffle=True, num_workers=0)
